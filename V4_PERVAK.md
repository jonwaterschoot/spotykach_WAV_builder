...
I worked through the open items. Everything that could be done without a browser is done and committed in five commits on v4-pervak.

Closed
A — storage namespacing (the blocker on any Pages deploy). New storageNamespace.ts; both IDB names and all 18 localStorage sites route through it. Namespace = VITE_STORAGE_NS → last BASE_URL segment when there's more than one → ''. The '' case matters most: the production bundle's names are byte-identical, so no existing user loses their handles. I checked both built bundles rather than reasoning about it — root computes '', next computes next. Added build:next/deploy:next.

D — auto-save. It genuinely didn't exist. Two things the plan didn't account for: AppState carries the audio blobs, so a snapshot isn't cheap — writes are serialised, newest state waiting while one is in flight, which self-limits to one write per write-duration. And it's gated on the mount-time load resolving, or the empty initial state overwrites the snapshot it's about to be replaced by. Exit-to-hub routes through the unsaved check, with different wording depending on the setting (with auto-save on, "these will be lost" would be false).

E, H(half) — the Project Manager. Two-column mirror → one list. ProjectSyncModal lost its last caller and went (497 lines). Took your recommended half on H: dead states stop rendering, the type keeps its four values, rename stays a separate mechanical commit.

F — workspace backup. workspaceBackup.ts + a modal. Itemised survey with sizes shown before a destination is even asked for; picker fires at that moment and the handle is never stored. On the size question it says plainly that the platform can't report free space, and fails cleanly instead — one folder, removed on failure.

G — the pen on a browser row, which pools and opens the editor in one step.

Step 1 — Settings. Locations, auto-save, backup, cleanup moved in; that's seven sections, so tabs (Files/Look/System). Cleanup left the Danger Zone — since the two-version rule it only removes leftovers.

N — docs. deployment_guidelines.md rewritten with Appendix F.2/F.3 folded in, so the deployment story survives V4_PERVAK being archived. v4 written up as one CHANGELOG entry.

One thing outside the plan: FileSystemDirectoryHandle's async iterators are now declared once in vite-env.d.ts instead of @ts-ignored at ~20 sites. It immediately caught a real narrowing gap in scanForProjects the suppression was hiding.

Types and build clean throughout; every new file lints clean and no existing file gained an error (App.tsx 82→79).

Still open — and these need you
Step 6, the test pass — the only thing left blocking v4. I can't run it; it needs a real card, a real folder picker, and a browser without showDirectoryPicker. Beyond the existing list, this round added four paths that have never executed: the auto-save loop under a real edit session (the guard bounds its cost, I didn't measure it), a workspace backup onto a card including one that runs out of room mid-write — the rollback is the whole point of that surface and has never run — the Project Manager against a card that already carries projects, and the pen.

Item O — the wireframing boxes. Three of the four (hub, independent editor, browser-to-grid) are built and shipping. My read: close them as built, since a sketch now would describe rather than decide. The fourth is question 6 below. Your call.

Question 6 — preset & pack authoring. The constraint that decides it: manifest.json is generated by a repo-side script, so without a backend "publishing" is a commit, not an upload. That rules out self-publishing regardless of which surface you build. My proposal: extend Studio's existing project export with a "export as preset submission" path — it already has the 36 files, tape names, notes and per-file metadata, and an artist who can't use Studio can't produce a valid preset anyway. A dedicated authoring surface is a second app to maintain for a flow that ends in a pull request either way. That leaves one real sub-question for you: do user-authored presets aim for the public manifest at all, or stay shareable-as-a-file?

Question 7 — SK1/, SK2/. Blocked on @Vlad, and the next step is the conversation, not code. One thing worth doing before any answer arrives, though: 'SK' is hardcoded in 13 places across 6 files. Making it a parameter is mechanical and safe now, and would be tangled up with feature work later — the same shape as Phase 4's backupHandle → sdHandle. Say the word and I'll do it as its own commit.
