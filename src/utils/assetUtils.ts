...
// External sample asset base URL (e.g., pointing to GitHub Releases)
const DEFAULT_SAMPLE_BASE_URL = 'https://github.com/jonwaterschoot/spotykach_WAV_builder/releases/download/samples-v1';
 ...
 * 2. Audio samples (/samples/...) are resolved against VITE_SAMPLE_ASSET_BASE_URL if set.
...
export const resolveAssetPath = (path: string): string => {
    ...
    if (audioSamplePathPattern.test(path)) {
        ...
        if (externalSampleAssetBaseUrl && fileName) {
